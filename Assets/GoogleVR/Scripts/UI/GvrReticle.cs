...
public class GvrReticle : MonoBehaviour, IGvrGazePointer {
  ...
  public float reticleGrowthSpeed = 8.0f;
	...
	// El eventSystem que manejará los inputs que no tengan que ver con el gaze
	public GameObject eventSystem;

	// Private members
	private Material materialComp;
	private GameObject targetObj;
	private bool reticleExpanded = false;
  ...
  public void OnGazeStay(Camera camera, GameObject targetObject, Vector3 intersectionPosition,
        ...
        //Debug.Log("Target object es: " + targetObject);

        // Then we try to get its ReticleProperties to know if we should expand the reticle or not.
        ReticleProperties other = (ReticleProperties)targetObject.GetComponent(typeof(ReticleProperties));
        if (other == null)
            return;

        //Debug.Log("Calling other: " + other);
        //Debug.Log("Max distance: " + other.MAX_DISTANCE);

        // If the distance is lower than the target's max distance, we allow the gaze to expand.
        if (Vector3.Distance(targetObject.transform.position, transform.position) <= other.MAX_DISTANCE)
        {
            expandReticle();
        }
        else
  ...
  public void OnGazeTriggerStart(Camera camera) {
		...
		// Si la retícula no está expandida, entonces decirle al controlador de escenas que es un click global
		if (!reticleExpanded)
		{
			GlobalInputHandler other = (GlobalInputHandler)eventSystem.GetComponent(typeof(GlobalInputHandler));
			other.onClick();
		}
  }
  ...
  private void CreateReticleVertices() {
    ...
    private void SetGazeTarget(Vector3 target, bool interactive)
    {
        ...
        // If distance between the target and our current position is less than MAX_DISTANCE, then we allow the reticle to expand.
       // if (Vector3.Distance(target, transform.position) <= MAX_DISTANCE)
       // {
            reticleDistanceInMeters =
	...
	public bool isReticleExpanded()
	{
		return reticleExpanded;
	}
}
